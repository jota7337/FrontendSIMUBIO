...
const ESPECIES_COLUMNS = new Set([
  "occurrenceID","basisOfRecord","type","institutionCode","institutionID","collectionCode","collectionID","catalogNumber","datasetName","datasetID","modified","language","license","rightsHolder","accessRights","bibliographicCitation","references","ownerInstitutionCode","informationWithheld","dataGeneralizations","dynamicProperties","recordNumber","recordedBy","recordedByID","organismID","individualCount","organismQuantity","organismQuantityType","organismName","organismScope","associatedOrganisms","previousIdentifications","organismRemarks","sex","lifeStage","reproductiveCondition","behavior","establishmentMeans","degreeOfEstablishment","pathway","occurrenceStatus","preparations","disposition","otherCatalogNumbers","associatedMedia","associatedOccurrences","associatedReferences","associatedSequences","associatedTaxa","occurrenceRemarks","materialSampleID","parentEventID","eventID","samplingProtocol","sampleSizeValue","sampleSizeUnit","samplingEffort","eventDate","startDayOfYear","endDayOfYear","year","month","day","verbatimEventDate","eventTime","habitat","fieldNumber","fieldNotes","eventRemarks","locationID","higherGeography","higherGeographyID","continent","waterBody","islandGroup","island","country","countryCode","stateProvince","county","municipality","locality","verbatimLocality","verbatimElevation","minimumElevationInMeters","maximumElevationInMeters","verticalDatum","verbatimDepth","minimumDepthInMeters","maximumDepthInMeters","minimumDistanceAboveSurfaceInMeters","maximumDistanceAboveSurfaceInMeters","locationAccordingTo","locationRemarks","verbatimLatitude","verbatimLongitude","verbatimCoordinates","verbatimCoordinateSystem","verbatimSRS","decimalLatitude","decimalLongitude","geodeticDatum","coordinateUncertaintyInMeters","coordinatePrecision","pointRadiusSpatialFit","footprintWKT","footprintSRS","footprintSpatialFit","georeferencedBy","georeferencedDate","georeferenceProtocol","georeferenceSources","georeferenceVerificationStatus","georeferenceRemarks","geologicalContextID","earliestEonOrLowestEonothem","latestEonOrHighestEonothem","earliestEraOrLowestErathem","latestEraOrHighestErathem","earliestPeriodOrLowestSystem","latestPeriodOrHighestSystem","earliestEpochOrLowestSeries","latestEpochOrHighestSeries","earliestAgeOrLowestStage","latestAgeOrHighestStage","lowestBiostratigraphicZone","highestBiostratigraphicZone","lithostratigraphicTerms","group","formation","member","bed","identificationID","identifiedBy","identifiedByID","dateIdentified","identificationReferences","identificationVerificationStatus","typeStatus","verbatimIdentification","identificationRemarks","identificationQualifier","scientificName","scientificNameAuthorship","taxonID","scientificNameID","higherClassification","kingdom","phylum","class","order","family","subfamily","genus","genericName","subgenus","infragenericEpithet","specificEpithet","infraspecificEpithet","cultivarEpithet","taxonRank","verbatimTaxonRank","vernacularName","taxonomicStatus","acceptedNameUsage","acceptedNameUsageID","parentNameUsage","parentNameUsageID","originalNameUsage","originalNameUsageID","nameAccordingTo","nameAccordingToID","namePublishedIn","namePublishedInID","namePublishedInYear","taxonConceptID","nomenclaturalCode","nomenclaturalStatus","taxonRemarks"
]);
...
const MAP = {
  ...
  departamento: "stateProvince", stateprovince: "stateProvince",
  county: "county",
  municipio: "municipality", municipality: "municipality",
  localidad: "locality", locality: "locality",
  ...
  lat: "decimalLatitude", latitude: "decimalLatitude", decimallatitude: "decimalLatitude",
  lon: "decimalLongitude", longitud: "decimalLongitude", longitude: "decimalLongitude", decimallongitude: "decimalLongitude",
...
function excelDateToISO(value) {
  ...
  // Si es string, déjalo tal cual (tu tabla es text)
  return String(value);
}
...
export async function parseExcelToEspeciesRows(fileOrBuffer, sheetName = "plantilla") {
  ...
  const headerMap = buildHeaderMap(rowsRaw[0] || {});
  const transformed = transformRows(rowsRaw, headerMap);
 ...
 */
export async function insertRowsIntoEspecies(supabase, rows, userId, batchSize = 200) {
  ...
  const ready = rows.map((row) => {
    ...
    return { ...filtered, created_by: userId };
  });
 ...
 */
export async function processAndInsertEspecies(fileOrBuffer, { supabase, sheetName = "plantilla", batchSize = 200, getUserId } = {}) {
  if (!supabase) throw new Error("Supabase client requerido.");
  const rows = await parseExcelToEspeciesRows(fileOrBuffer, sheetName);
  ...
  const res = await insertRowsIntoEspecies(supabase, rows, userId, batchSize);
