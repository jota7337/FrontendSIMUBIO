      ...
      <div className="w-full max-w-5xl bg-white rounded-2xl shadow-2xl p-8 border border-green-300">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <img src={logo} alt="Logo Universidad El Bosque" className="h-16 w-16 rounded-full border-2 border-green-700 bg-yellow-300" />
            <div>
        ...
        {!loading && (
          <div className="mb-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="bg-green-50 border border-green-200 rounded-xl p-6 flex flex-col items-center shadow">
              <span className="text-2xl font-bold text-green-800">{referenceStats.reduce((acc, r) => acc + r.cantidad, 0)}</span>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6 flex flex-col items-center shadow">
              <span className="text-2xl font-bold text-yellow-700">{referenceStats.reduce((acc, r) => acc + r.aprobadas, 0)}</span>
            ...
            <div className="bg-green-100 border border-green-300 rounded-xl p-6 flex flex-col items-center shadow">
              <span className="text-2xl font-bold text-green-900">
                {referenceStats.length > 0 ? (
                  ((referenceStats.reduce((acc, r) => acc + r.aprobadas, 0) / referenceStats.reduce((acc, r) => acc + r.cantidad, 0)) * 100).toFixed(1)
                ) : 0}%
              </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {referenceStats.map((ref, idx) => (
                  <div key={idx} className="p-4 rounded-xl border border-green-200 bg-green-50 shadow flex flex-col items-center">
                    ...
                    <span className="text-sm font-semibold text-yellow-700 bg-yellow-200 px-2 py-1 rounded mt-2">Aprobadas: {ref.aprobadas}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="flex flex-col gap-8 items-center justify-center">
              <div className="bg-white rounded-xl border border-yellow-200 shadow p-4">
                <h2 className="text-lg font-bold text-yellow-700 mb-2 text-center">Registro de Especies por Referencia</h2>
              ...
              <div className="bg-white rounded-xl border border-green-200 shadow p-4">
                <h2 className="text-lg font-bold text-green-700 mb-2 text-center">Especies Aprobadas por Referencia</h2>
                <PieChart width={350} height={220}>
                  <Pie data={dataPie} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} fill="#fbc02d" label />
                  <Tooltip />
                </PieChart>
              </div>
            </div>
