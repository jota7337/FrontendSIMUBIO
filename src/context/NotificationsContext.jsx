  ...
  return (
    <NotificationsContext.Provider value={api}>
      ...
      <div className='fixed z-50 top-4 right-4 space-y-3 w-80'>
        {toasts.map(t => (
          <div key={t.id} className={`rounded-lg shadow-lg border p-4 text-sm backdrop-blur bg-white/90 border-green-200 animate-[fadeIn_.25s_ease]`}>            
            <div className='flex justify-between items-start gap-3'>
              ...
              <button onClick={() => remove(t.id)} className='text-green-700 hover:text-green-900'>✕</button>
            </div>
            <div className={`mt-2 h-1 rounded bg-gradient-to-r from-green-600 via-yellow-400 to-green-400 animate-[shrink_${(t.duration||autoHide)/1000}s_linear_forwards]`}></div>
          </div>
        ))}
      </div>
      <style>{`
